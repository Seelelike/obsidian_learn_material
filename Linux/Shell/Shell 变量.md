> 本文由 [简悦 SimpRead](http://ksria.com/simpread/) 转码， 原文地址 [www.runoob.com](https://www.runoob.com/linux/linux-shell-variable.html)
...
**等号两侧避免使用空格**：
>Shell将命令行解析为**命令 + 参数**的形式。空格是参数的分隔符。
```
# 正确的赋值
variable_name=value
...
这样写是合法的，但注意，第二次赋值的时候不能写\$your_，使用变量的时候才加美元符（\$）。
