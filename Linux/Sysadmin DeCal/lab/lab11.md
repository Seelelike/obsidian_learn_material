> 本文由 [简悦 SimpRead](http://ksria.com/simpread/) 转码， 原文地址 [decal.ocf.berkeley.edu](https://decal.ocf.berkeley.edu/archives/2021-fall/labs/11/)
...
1.  [Overview](#overview)
    ...
    2.  [The most important trick with puppet](#the-most-important-trick-with-puppet)
2.  [Part 1: Installing Puppet](#part-1-installing-puppet)
3.  [Part 2: Using Puppet](#part-2-using-puppet)
    1.  [Part a: Making a Home Directory and a User](#part-a-making-a-home-directory-and-a-user)
        1.  [Checking Part a](#checking-part-a)
    2.  [Part b: Install java](#part-b-install-java)
        1.  [Checking Part 2b](#checking-part-2b)
    3.  [Part c: Installing the Minecraft Server configuration](#part-c-installing-the-minecraft-server-configuration)
        1.  [Checking Part c](#checking-part-c)
    4.  [Part d: Installing the Minecraft Server](#part-d-installing-the-minecraft-server)
        1.  [Checking Part d](#checking-part-d)
    5.  [Part e: Templating a systemd unit file](#part-e-templating-a-systemd-unit-file)
        1.  [Checking Part e](#checking-part-e)
    6.  [Part f: Running the service](#part-f-running-the-service)
        1.  [Checking Part f](#checking-part-f)
    7.  [Part g: Backups](#part-g-backups)
        1.  [Checking Part g](#checking-part-g)
    8.  [Part h: (Bonus, optional if you want Minecraft to not Out Of Memory)](#part-h-bonus-optional-if-you-want-minecraft-to-not-out-of-memory)
        1.  [Checking Part h](#checking-part-h)
    9.  [Part i: (Bonus, running Minecraft)](#part-i-bonus-running-minecraft)
4.  [Part 3: Cleanup](#part-3-cleanup)
5.  [Part 4: Submission](#part-4-submission)
...
For this lab, we will be installing and configuring a Minecraft server. All of this configuration can be sucessfully done without a Minecraft client or knowledge on how to play the game itself.

**If you want to actually be able to connect to the server:** The student VMs do not have enough RAM to run a Minecraft server, so you’ll need to complete parts h) and i) if you plan on hosting your server on your VM. Alternatively, you are welcome to complete this lab on your own machine if it has at least 8GB of RAM, in which case parts h) and i) may not be necessary.

[](#getting-help)Getting help
-----------------------------

If you want any help with any part of this lab, join the OCF slack ([https://ocf.io/slack](https://ocf.io/slack)) or Discord ([https://ocf.io/discord](https://ocf.io/discord))!
...
[](#the-most-important-trick-with-puppet)The most important trick with puppet
-----------------------------------------------------------------------------
...
[](#part-1-installing-puppet)Part 1: Installing Puppet
======================================================
...
[](#part-2-using-puppet)Part 2: Using Puppet
============================================
...
Useful references are this section are the [Puppet documentation](https://puppet.com/docs/puppet/5.5/puppet_index.html) and there is a lot of sample code avaliable in the [OCF Puppet configuration](https://github.com/ocf/puppet). When you are stuck, looking at existing code and see how they did things will generally be helpful. Also remember that there are code examples on the [slides](https://docs.google.com/presentation/d/1OfIJHu6bSfTOLLswpKnVr7oxL8R-H4aPM8LLRHuecCY/edit)!

[](#part-a-making-a-home-directory-and-a-user)Part a: Making a Home Directory and a User
----------------------------------------------------------------------------------------
...
Inside `minecraft.pp`, write some Puppet code to create a user named `minecraft`, that the Minecraft server will run under. The `minecraft` user should have home directory `/home/minecraft`. Now modify the code creating `/home/minecraft` to set owner the owner to `minecraft` user you just made. Check the example code on the [slides](https://docs.google.com/presentation/d/1OfIJHu6bSfTOLLswpKnVr7oxL8R-H4aPM8LLRHuecCY/edit) if you are unsure about how to do this.

### [](#checking-part-a)Checking Part a

Run `sudo puppet apply minecraft.pp`.

Now run `ls -l /home` and verify that `/home/minecraft` is owned by the minecraft user.

[](#part-b-install-java)Part b: Install java
--------------------------------------------

Add a few lines to `minecraft.pp` to install the `default-jre` package.

### [](#checking-part-2b)Checking Part 2b

Run `java` and verify that the binary exists.

[](#part-c-installing-the-minecraft-server-configuration)Part c: Installing the Minecraft Server configuration
--------------------------------------------------------------------------------------------------------------

Copy paste the contents of [https://raw.githubusercontent.com/0xcf/decal-labs/master/a10/server.properties](https://raw.githubusercontent.com/0xcf/decal-labs/master/a10/server.properties) locally into a file named `server.properties`
...
Read and agree to the [Minecraft EULA](https://account.mojang.com/documents/minecraft_eula), and ensure that `/home/minecraft/eula.txt` contains the text `eula=true` by hardcoding the string `eula=true` into your `minecraft.pp`.

Make sure that all of the above files are owned by the `minecraft` user.

### [](#checking-part-c)Checking Part c
...
[](#part-d-installing-the-minecraft-server)Part d: Installing the Minecraft Server
----------------------------------------------------------------------------------

Ensure that /home/minecraft/server.jar contains the Minecraft Server, available at [https://launcher.mojang.com/v1/objects/3dc3d84a581f14691199cf6831b71ed1296a9fdf/server.jar](https://launcher.mojang.com/v1/objects/3dc3d84a581f14691199cf6831b71ed1296a9fdf/server.jar).
...
### [](#checking-part-d)Checking Part d

You know what to do!

[](#part-e-templating-a-systemd-unit-file)Part e: Templating a systemd unit file
--------------------------------------------------------------------------------
...
Hint: In the [slides](https://docs.google.com/presentation/d/1OfIJHu6bSfTOLLswpKnVr7oxL8R-H4aPM8LLRHuecCY/edit)), there is an example of templating a file.
...
### [](#checking-part-g)Checking Part g

Look in the `/home/minecraft/backups` contains backups!

[](#part-h-bonus-optional-if-you-want-minecraft-to-not-out-of-memory)Part h: (Bonus, optional if you want Minecraft to not Out Of Memory)
-----------------------------------------------------------------------------------------------------------------------------------------
...
### [](#checking-part-h)Checking Part h
...
[](#part-i-bonus-running-minecraft)Part i: (Bonus, running Minecraft)
---------------------------------------------------------------------
...
[](#part-4-submission)Part 4: Submission
========================================
